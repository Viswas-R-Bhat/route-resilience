...
// Criticality tier color mappings
const TIERCOL = { 
  critical: "#ff6b5e", 
  important: "#f0b75e", 
  normal: "#5ec8a0" 
...
/* =========================================================================
   1. GRAPH & RESILIENCE MATH (Mirrors Phase IV Analysis)
   ========================================================================= */
...
// Calculate the network efficiency index based on active nodes
function efficiency(disabled) {
  ...
  for (const s of live) {
    ...
    for (const t of live) {
      if (t !== s && dist[t] < Infinity && dist[t] > 0) {
        total += 1 / dist[t];
      }
    }
  }
...
// Calculate the fraction of nodes belonging to the Largest Connected Component (LCC)
...
// Identify road nodes that are currently submerged under the water level
function floodSubmerged() { // road nodes below the current waterline
  ...
  return DATA.nodes
    .filter(n => n.elev != null && n.elev <= lvl)
    .map(n => n.id);
}

// Get the unified set of all currently disabled nodes (ablation + manual + flooded)
const disabledSet = () => new Set([
  ...ABL_ORDER.slice(0, topN), 
  ...manual, 
  ...floodSubmerged()
]);
...
/* =========================================================================
   2. SHORTEST PATHS & ROUTING CALCULATIONS (PS-4 Phase IV)
   ========================================================================= */

// Heap-based Dijkstra algorithm for single-source shortest paths
...
// Reconstruct path from origin to destination using parent pointers
...
// Choose a representative O-D route to demonstrate rerouting
// and sample sector pairs to calculate general travel-time increases
function computeRoutes() {
  ...
  for (const s of IDS) {
    for (const t of IDS) {
      if (t === s || DIST[s][t] === Infinity || (g != null && (s === g || t === g))) {
        continue;
      }
  ...
  // Fallback: use plain weighted diameter if no path contains the top gatekeeper
  if (O == null) { 
    for (const s of IDS) {
      for (const t of IDS) {
        if (t !== s && DIST[s][t] < Infinity && DIST[s][t] > best) { 
          best = DIST[s][t]; 
          O = s; 
          D = t; 
        }
  ...
  OD = [O, D]; 
  BASE_OD_PATH = pathOf(PREV[O], D);
  
  const all = [];
  for (const s of IDS) {
    for (const t of IDS) {
      if (t > s && DIST[s][t] < Infinity) {
        all.push([s, t, DIST[s][t]]);
      }
    }
  }
  
  const stepp = Math.max(1, Math.floor(all.length / 40));
  for (let i = 0; i < all.length; i += stepp) {
    SECTOR.push(all[i]);
  }
}
...
// Generate HTML string for KPI cards
...
// Main rendering entrypoint: calculates metrics and updates the UI components
function render() {
  const fset = new Set(floodSubmerged());
  const dis = disabledSet();
  
  // If only one stressor is active, use the matching precomputed Python curve (instant at any size)
  let pre = null;
  if (floodStep > 0 && topN === 0 && manual.size === 0 && DATA.flood && DATA.flood[floodStep]) {
    pre = DATA.flood[floodStep];
  } else if (floodStep === 0 && manual.size === 0 && DATA.resilience && DATA.resilience[topN]) {
    pre = DATA.resilience[topN];
  }
  ...
  // Update header / sidebar status info
  $("#ri-num").textContent = R.toFixed(2); 
  $("#ri-num").style.color = col; 
  $("#ri-num").style.textShadow = `0 0 34px ${col}40`;
  
  $("#ri-status").textContent = status; 
  $("#ri-status").style.color = col;
  ...
  // Flood readout calculations (water level + submerged share)
  const fm = $("#flood-m");
  const fsub = $("#flood-sub");
  ...
  let ttTxt = "+0%";
  let ttSub = `${SECTOR.length} sector routes`;
  let ttCol = col;
  ...
  if (SECTOR.length) {
    ...
    if (dis.size === 0) { 
      ttTxt = "+0%"; 
      ttSub = `${SECTOR.length} sector routes`; 
      ttCol = "#5ec8a0"; 
    } else if (inc.length === 0) { 
      ttTxt = "cut off"; 
      ttSub = `all ${sev} routes severed`; 
      ttCol = "#ff6b5e"; 
    } else if (sev > inc.length) { // severing dominates -> lead with it (honest)
      ttTxt = `${cutPct}% cut`; 
      ttSub = `+${Math.round(mean * 100)}% on ${inc.length} open routes`; 
      ttCol = "#ff6b5e";
  ...
  const ttn = $("#tt-num"); 
  if (ttn) { 
    ttn.textContent = ttTxt; 
    ttn.style.color = ttCol; 
    ttn.style.textShadow = `0 0 30px ${ttCol}40`; 
  }
  
  const tts = $("#tt-sub"); 
  if (tts) { 
    tts.textContent = ttSub; 
    tts.style.color = ttCol; 
  }

  // Toggle off-state on gatekeeper rows
  document.querySelectorAll(".gk-row").forEach(r => {
    r.classList.toggle("off", dis.has(+r.dataset.id));
  });
  
  // Update line chart cursor position marker
  chart.setOption({ 
    series: [{ 
      markLine: { 
        silent: true, 
...
/* =========================================================================
   4. GATEKEEPER TABLES & INTERACTIBILITY
   ========================================================================= */

// Render the list of top gatekeeper nodes (Betweenness or Service Criticality)
function renderGatekeepers() {
  ...
  $("#gatekeepers").innerHTML = list.map((g, i) => {
    ...
    return `<div class="gk-row" role="listitem" data-id="${id}" tabindex="0">` +
       ...
       `<span class="gk-id">N-${id}</span>` +
       `<span class="gk-bc">${metric}</span>` +
       `</div>`;
  }).join("");
...
/* =========================================================================
   5. RESILIENCE CHART (ECHARTS)
   ========================================================================= */

// Render the resilience curve and LCC fraction line chart
function renderChart() {
  ...
  chart.setOption({
    ...
    xAxis: { 
      ...
      data: steps, 
      name: "nodes removed", 
      nameTextStyle: { color: "#6e6e73", fontSize: 9 }, 
    ...
    series: [
      { 
        ...
        symbol: "circle", 
        symbolSize: 6, 
        lineStyle: { color: "#5ec8a0", width: 3 }, 
        itemStyle: { color: "#5ec8a0" }, 
        areaStyle: { color: "rgba(94,200,160,.14)" } 
      },
      { 
        name: "LCC", 
        type: "line", 
        ...
        smooth: true, 
        symbol: "none", 
        lineStyle: { color: "#f0b75e", width: 2, type: "dashed" }, 
        itemStyle: { color: "#f0b75e" } 
      },
    ],
  }, true);
}
...
/* =========================================================================
   6. OPENSTREETMAP COMPARISON METRICS
   ========================================================================= */

// Render precision, recall, and routing comparison metrics compared to OSM data
function renderOsm() {
  const card = $("#osm-card"); 
  if (!card) return;
  ...
  card.hidden = false;
  const c = o.coverage;
  const p = o.path_length || {};
  
  const cell = (val, label, sub, accent) => `
    <div class="osm-metric">
      <span class="om-val" style="color:${accent}">${val}</span>
      <span class="om-label">${label}</span>
      <span class="om-sub">${sub}</span>
    </div>
  `;
...
/* =========================================================================
   7. INTERACTIVE NODE SELECTION & MANUAL TOGGLES
   ========================================================================= */

// Toggle a node manually to disable/enable it on mouse clicks
...
/* =========================================================================
   8. REGIONAL DATA TILE LOADER
   ========================================================================= */

// Fetch and load geospatial/topological data for a specific area/tile
async function loadTile(stem) {
  ...
  const slider = $("#ablate"); 
  slider.max = ABL_ORDER.length; 
  slider.value = 0;
  
  // Flood control: only when this tile has a DEM-derived flood curve
  const hasFlood = !!(DATA.flood && DATA.flood.length > 1);
  ["#flood-wrap", "#flood", "#flood-sub"].forEach(s => { 
    const el = $(s); 
    if (el) el.hidden = !hasFlood; 
  });
  
  if (hasFlood) { 
    const f = $("#flood"); 
    f.max = DATA.flood.length - 1; 
    f.value = 0; 
  }
  
  const gkb = $("#gk-mode"); 
  if (gkb) {
    gkb.hidden = !(DATA.service_gatekeepers && DATA.service_gatekeepers.length);
  }
  ...
  if (geo) {
    ...
    if (!mapView) {
      mapView = new MapView("map", id => toggleManual(id));
    }
    activeView = mapView;
  } else {
  ...
  if (tools) {
    tools.style.display = geo ? "none" : "flex";
  }
  
  const hint = document.querySelector(".scene-hint");
  if (hint) {
    hint.textContent = geo ? "drag to pan · scroll to zoom · click a node to disable it"
  ...
  if (activeView.setOsm) {
    activeView.setOsm(null);
  }
  
  const ot = $("#osm-toggle"); 
  if (ot) { 
    ot.setAttribute("aria-pressed", "false"); 
    ot.textContent = "Show roads"; 
  }
...
/* =========================================================================
   9. DYNAMIC LIVE ANALYSIS (API CONNECTIONS & MODEL PIPELINE)
   ========================================================================= */

// Display loading state feedback message
...
// Geocode an input query string into coordinates using OSM Nominatim API
...
// Main logic for sending an input place name/coordinates to backend for live ML model inference
async function analyzeLocation() {
  ...
  if (m) { 
    ...
    let g = null;
    try { 
      g = (await geocode(raw)) || (await geocode(raw + ", Bengaluru, Karnataka, India")); 
    } catch (e) {
      // Ignored geocode error
    }
    
    if (!g) { 
      setLoader("Place not found — try “lat, lon”", true); 
      setTimeout(() => setLoader("", false), 2400); 
      return; 
    }
    lat = g.lat; 
    lon = g.lon;
  ...
  console.log("Search input:", raw, "-> Coordinates:", lat, lon);
  ...
  try {
    ...
    if (j.error) { 
      setLoader(`Analysis failed: ${j.error}`, true); 
    ...
    if (![...sel.options].some(o => o.value === j.stem)) {
      ...
      opt.value = j.stem; 
      opt.textContent = `Live · ${(+lat).toFixed(3)}, ${(+lon).toFixed(3)} · map`;
...
/* =========================================================================
   10. INTERACTION CONTROLS WIRING & INITIALIZATION
   ========================================================================= */

// Wire user controls (sliders, buttons, option selections) and bootstrap the UI
async function init() {
  const man = await (await fetch("data/manifest.json?t=" + Date.now())).json();
  const sel = $("#region");
  
  sel.innerHTML = man.tiles
    .map(t => `<option value="${t.stem}">${t.label || t.stem} · ${t.nodes} nodes${t.geo ? " · map" : ""}</option>`)
    .join("");
  ...
  $("#flood").oninput = e => { 
    floodStep = +e.target.value; 
    render(); 
  };
  
  $("#reset-stress").onclick = () => { 
    topN = 0; 
    manual.clear(); 
    floodStep = 0;
    $("#ablate").value = 0; 
  ...
  if (gkb) {
    gkb.onclick = () => { 
      gkMode = gkMode === "bc" ? "svc" : "bc";
      gkb.setAttribute("aria-pressed", gkMode === "svc"); 
      gkb.textContent = gkMode === "svc" ? "Service" : "Betweenness";
      renderGatekeepers(); 
      render(); 
    };
  }
  ...
  if (ot) {
    ot.onclick = () => { 
      osmShown = !osmShown; 
      ot.setAttribute("aria-pressed", osmShown);
      ot.textContent = osmShown ? "Hide roads" : "Show roads";
      
      if (activeView.setOsm) {
        activeView.setOsm(osmShown ? (DATA.osm_lines || []) : null); 
      }
    };
  }
  ...
  tr.onclick = () => { 
    ...
    tr.setAttribute("aria-pressed", on); 
    tr.textContent = `Auto-orbit: ${on ? "On" : "Off"}`; 
    activeView.setAutoRotate(on); 
  };
  
  const th = $("#toggle-heat");
  th.onclick = () => { 
    const on = th.getAttribute("aria-pressed") !== "true"; 
    th.setAttribute("aria-pressed", on); 
    th.textContent = `Criticality heat: ${on ? "On" : "Off"}`; 
    activeView.setHeat(on); 
    render(); 
  };
...
// Entrypoint initialization with fallback error reporting
init().catch(e => { 
  console.error(e); 
  $("#loader").innerHTML = "<span>Failed to load data. Run src/export_web.py</span>"; 
});
